...
async function loadHistory() {
  ...
  if (!token) {
    errorMessage.value = "Admin login is required to view stock history."
  ...
  try {
    history.value = await getInventoryHistory(token)
  } catch (error) {
    errorMessage.value = error.message || "Unable to load stock history right now."
  } finally {
    isLoading.value = false
  }
}
...
<template>
  <div class="history-page">
    ...
    <div v-else-if="isLoading" class="notice">
      Loading stock history from the backend...
    </div>
    ...
    <div v-else class="panel">
      ...
      <div class="item" v-for="h in history" :key="h.id">
        <div class="left">
          <div :class="['icon', h.change > 0 ? 'green' : 'red']">
            {{ h.change > 0 ? '+' : '-' }}
          </div>
        ...
        <div class="right">
          <h3 :class="h.change > 0 ? 'green-text' : 'red-text'">
            {{ h.change > 0 ? '+' : '' }}{{ h.change }} units
          </h3>

          <p class="range">{{ h.from }} -> {{ h.to }}</p>
        </div>
      </div>
    </div>
  </div>
